// app/dashboard/super-admin/directory/page.tsx
'use client';

import { useEffect, useState } from 'react';
import { LawFirmAdmin } from '../../../../models/superAdmin';
import { Search, Building, Users, Mail, Phone, Contact2, CheckCircle2, XCircle } from 'lucide-react';
...
export default function DirectoryPage() {
    ...
    const fetchAllLawFirms = async () => {
        setIsFirmsLoading(true); setFirmsError('');
        try {
            const data = await superAdminService.getAllLawFirms();
            setLawFirms(data);
        } catch (err: any) { setFirmsError(err.message); } 
        finally { setIsFirmsLoading(false); }
    };

    const handleFirmSearch = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!firmSearchQuery) return fetchAllLawFirms();
        setIsFirmsLoading(true); setFirmsError('');
        try {
        ...
        <div>
            <div className="p-6 border-b border-slate-100 bg-slate-50/50">
                <form onSubmit={handleFirmSearch} className="relative max-w-lg">
                    ...
                    <input 
                        ...
                        onChange={(e) => setFirmSearchQuery(e.target.value.toUpperCase())}
            ...
            <div className="p-6 bg-slate-50 min-h-[400px]">
                {isFirmsLoading ? (
                    ...
                    <div className="p-8 text-center text-red-600 font-medium bg-red-50 rounded-xl border border-red-100">{firmsError}</div>
                ) : lawFirms.length === 0 ? (
